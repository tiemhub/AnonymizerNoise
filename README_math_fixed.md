...
Given a source image `x`, we first generate an anonymized counterpart `x_anon`. The anonymized image is used as a **source-conditioned latent reference**, not as the final protected output.

Using a VAE encoder `E`, the anonymized reference is encoded as:

```math
z_{\mathrm{anon}} = E(x_{\mathrm{anon}})
```

For a pixel-space perturbation `δ`, the perturbed source image is encoded as:

```math
z_{\mathrm{perturbed}} = E(x + \delta)
```

The default formulation minimizes the latent distance between the perturbed source representation and the anonymization-derived reference:

```math
\mathcal{L}_{\mathrm{anon}}
=
\left\|
z_{\mathrm{perturbed}} - z_{\mathrm{anon}}
\right\|_2^2
```
...
The protected image is then defined as:
...
- the `L∞` constraint bounds pixel-wise perturbation magnitude but does not independently establish perceptual imperceptibility;
